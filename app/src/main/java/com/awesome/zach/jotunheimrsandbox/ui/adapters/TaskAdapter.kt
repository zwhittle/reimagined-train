...
class TaskAdapter(
    ...
    private val isMultiSelectionEnabled: Boolean) : RecyclerView.Adapter<TaskViewHolder>(), TaskViewHolder.OnTaskSelectedListener {
    ...
    override fun onBindViewHolder(holder: TaskViewHolder, position: Int) {
        ...
        holder.mTask = task
        holder.setChecked(holder.mTask.isSelected)
    }
    ...
    inner class TaskDiffCallback(private val tasks: List<Task>) : DiffUtil.Callback() {
        ...
        override fun areContentsTheSame(
            ...
            return newTask.taskId == oldTask?.taskId
                && newTask.name == oldTask.name
                && newTask.date_start == oldTask.date_start
                && newTask.date_end == oldTask.date_end
                && newTask.completed == oldTask.completed
                && newTask.priority == oldTask.priority
                && newTask.projectId == oldTask.projectId
        }
    }
}